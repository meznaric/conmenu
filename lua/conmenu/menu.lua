...
local function showMenu()
	...
	vim.api.nvim_buf_set_keymap(state.bufh, "n", "<LeftMouse>", "", {
		noremap = true,
		silent = true,
		callback = function()
			local mouse_pos = vim.fn.getmousepos()

			-- Just removes the warning, but I never found the case when it was nil
			if mouse_pos == nil then
				return
			end

			-- Pressed outside of menu
			if mouse_pos.winid ~= winId then
				require("conmenu").close()
				return
			end

			local line = mouse_pos.line
			local item = state.activeCommands[line]
			if item ~= nil then
				require("conmenu").executeItemNum(line)
			end
		end,
	})
		...
		if commandOrMenu ~= nil then
			...
			if result[1] ~= nil then
				vim.keymap.set(
					"n",
					result[1],
					'<Cmd>lua require("conmenu").executeItemNum(' .. index .. ")<CR>",
...
-- Filters current menu commands based on provided options.
local function filterActiveCommands()
	...
	for i, v in ipairs(state.activeCommands) do
		...
		if options == nil then
			...
			local filetypeValid = not options.onlyTypes and true
				or utils.listIncludes(options.onlyTypes, vim.bo.filetype)
...
local function open()
	...
	state.relative = vim.g["conmenu#relative"]
	state.activeCommands = vim.g["conmenu#default_menu"]
	state.row = vim.g["conmenu#row"]
	state.col = vim.g["conmenu#col"]
	filterActiveCommands()
	showMenu()
end

-- This let's you open custom menu from your lua/vim script
local function openCustom(menu, windowOptions)
	windowOptions = windowOptions or {}
	if state.bufh ~= nil then
		close()
	end
	state.relative = windowOptions.relative or vim.g["conmenu#relative"]
	state.row = windowOptions.row or vim.g["conmenu#row"]
	state.col = windowOptions.col or vim.g["conmenu#col"]
